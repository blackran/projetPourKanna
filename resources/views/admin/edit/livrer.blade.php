...
@section("containts")
  <div>
    ...
    <div class="HEAD">
      LIVRAISON N° {{ $livrer->NumLiv }}
    </div>
    <div class="contains">
      <div>
        RESPONSABLE N°: {{ $livrer->NumResp }}<br>
        REF COMPOSANTE: {{ $livrer->RefComp }}<br>
        FACTURE N°: {{ $livrer->NumFct }}<br>
        DATE: {{ $livrer->DateLiv }}<br>
        QUANTITÉ: {{ $livrer->QtLiv }}<br>
        LIEU: {{ $livrer->LieuLiv }}
      </div>
      <a href="/config/{{ $column }}/{{ $name }}/{{ $livrer->NumLiv }}"><img src="{{ asset('/image/149-cog.png')}}" width="32" height="32"></a>
    </div>
    @endforeach
  </div>
@endsection
