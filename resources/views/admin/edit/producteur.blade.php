...
@section("containts")
  <div>
    ...
    <div class="HEAD">
      {{ $producteur->NumPdr }} - {{ $producteur->NomPdr }}
    </div>
    <div class="contains">
      <div>
        ADDRESS: {{ $producteur->AdressPdr }}<br>
        CODE POSTAL: {{ $producteur->CpPdr }}<br>
        VILLE: {{ $producteur->VillePdr }}<br>
        TEL: {{ $producteur->TelPdr }}
      </div>
      <a href="/config/{{ $column }}/{{ $name }}/{{ $producteur->NumPdr }}"><img src="{{ asset('/image/149-cog.png')}}" width="32" height="32"></a>
    </div>
    @endforeach
  </div>
@endsection
