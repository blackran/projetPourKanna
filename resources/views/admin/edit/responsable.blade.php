...
@section("containts")
  <div>
    ...
    <div class="HEAD">
      {{ $responsable->NumResp }} - {{ $responsable->NomResp }}
    </div>
    <div class="contains">
      <div>
        ADDRESS: {{ $responsable->AddessResp }}<br>
        PASSWORD: {{ $responsable->passResp }}<br>
      </div>
      <a href="/config/{{ $column }}/{{ $name }}/{{ $responsable->NumResp }}"><img src="{{ asset('/image/149-cog.png')}}" width="32" height="32"></a>
    </div>
    @endforeach
  </div>
@endsection
