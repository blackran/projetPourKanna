...
@section("containts")
  <div>
    ...
    <div class="HEAD">
      {{ $client->NumCli }} - {{ $client->CivilitéCli  }} {{ $client->NomCli }} {{ $client->PrenomCli }}
    </div>
    <div class="contains">
      <div>
        ADDRESS: {{ $client->AdressCli}}<br>
        CODE POSTAL: {{ $client->CpCli}}<br>
        VILLE: {{ $client->VilleCli}}<br>
        TEL:{{ $client->TelCli }}
      </div>
      <a href="/modif/{{ $column }}/{{ $name }}/{{ $client->NumCli }}"><img src="{{ asset('/image/149-cog.png')}}" width="32" height="32"></a>
    </div>
    @endforeach
  </div>
@endsection
